            ...
            for model_key, model in module.MODEL_SET.items():
                ...
                if model_hashes and model_sources:
                    for model_type, data in model_sources.items():
                        ...
                        hash_url = model_hashes.get(model_type, {}).get('url') # Fixed hash URL retrieval
                        hash_path = model_hashes.get(model_type, {}).get('path') # Fixed hash path retrieval

                        if url and file_path and hash_url and hash_path: # Ensure all required attributes exist before appending
